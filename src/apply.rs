...
use crate::parse::{
    matches_pattern, parse_cell_range, parse_cell_ref, parse_color, parse_color_enum,
    parse_column_format, parse_header_format, parse_horizontal_alignment, parse_icon_type,
    parse_rich_text_format, parse_vertical_alignment,
};
use crate::types::{
    CellWrite, CheckboxConfig, Comment, ConditionalFormatConfigs, Hyperlink, ImageConfig,
    MergedRange, RichTextSegment, TextboxConfig, ValidationConfig,
};
use indexmap::IndexMap;
use pyo3::prelude::*;
use rust_xlsxwriter::{
    Color, ConditionalFormat2ColorScale, ConditionalFormat3ColorScale, ConditionalFormatBlank,
    ...
    Shape, ShapeFont, ShapeFormat, ShapeLine, ShapeSolidFill, Worksheet,
};
...
pub(crate) fn apply_textboxes(
    ...
    for (cell_ref, (text, options)) in textboxes {
        let (row, col) = parse_cell_ref(cell_ref)?;

        let mut shape = Shape::textbox().set_text(text.as_str());

        let (x_offset, y_offset) = if let Some(opts) = options {
            for key in opts.keys() {
