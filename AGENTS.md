...
- **A new third-party import in a test cannot be validated locally.** The local `.venv` holds the whole `dev` extras, but the three `python-test*` jobs in CI install an explicit minimal list (`maturin pandas polars openpyxl pytest pyyaml`) and nothing else — so a test importing anything outside that list passes locally and fails only in CI with `ModuleNotFoundError`. Declaring the package in `[project.optional-dependencies] dev` does **not** fix it; the test jobs never install `dev`. Adding an import to `tests/` means editing those three `pip install` lines too (2026-07-30: `tests/test_docs_site.py` imported `yaml`, was declared in `dev`, and still broke three jobs).
